...
import { fetchAccount, updateProfile, changePassword } from '../features/auth/authSlice';
import {
  Box,
  Paper,
  Stack,
...
export default function Profile() {
  ...
  const { user, account, accountStatus } = useAppSelector((state) => state.auth);
  ...
  const handleEditSubmit = () => {
    ...
    if (!editForm.soDT.trim()) errors.soDT = 'Số điện thoại không được để trống';
    ...
    <Box sx={{ maxWidth: 1200, mx: 'auto' }}>
      ...
      >
        <Stack direction={{ xs: 'column', sm: 'row' }} alignItems="center" spacing={3}>
          ...
          >
            {loading ? <Skeleton variant="circular" width={100} height={100} /> : getInitials(account?.hoTen || user?.hoTen)}
          </Avatar>

          <Box sx={{ flex: 1, textAlign: { xs: 'center', sm: 'left' } }}>
            {loading ? (
              <Stack spacing={1}>
                <Skeleton width={200} height={40} sx={{ bgcolor: 'rgba(255,255,255,0.3)' }} />
                <Skeleton width={150} height={24} sx={{ bgcolor: 'rgba(255,255,255,0.3)' }} />
              </Stack>
            ) : (
              <>
                ...
                <Typography variant="body1" sx={{ opacity: 0.8, mt: 1 }}>
                  Thành viên CGV • Tham gia từ {dayjs().subtract(30, 'day').format('MM/YYYY')}
                </Typography>
              </>
            )}
          </Box>
            ...
            <Card sx={{ borderRadius: 3, boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
              <CardContent sx={{ p: 3 }}>
                ...
                <Stack spacing={2}>
                  ...
                  <Box sx={{ textAlign: 'center', p: 2, bgcolor: '#f8f9fa', borderRadius: 2 }}>
                    <Typography variant="h3" fontWeight={800} color="#667eea">
                      {account?.thongTinDatVe?.reduce((total, booking) => total + (booking.danhSachGhe?.length || 0), 0) || 0}
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                      Ghế đã đặt
                    </Typography>
      ...
      {tabValue === 1 && (
        <Box>
          <Stack direction="row" alignItems="center" justifyContent="space-between" mb={3}>
            <Typography variant="h5" fontWeight={700}>Lịch sử đặt vé</Typography>
            <Button
          ...
          {loading ? (
            ...
            <Stack spacing={3}>
              {account?.thongTinDatVe?.length ? account.thongTinDatVe.map((booking, idx) => (
                <Card key={idx} sx={{ borderRadius: 3, boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
                  <CardContent sx={{ p: 3 }}>
                    <Stack direction="row" alignItems="flex-start" spacing={3}>
                      ...
                      <Box sx={{ flex: 1 }}>
                        ...
                        <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 2 }}>
                          ...
                          <Typography variant="body2" color="text.secondary">
                            Đặt lúc: {booking.ngayDat ? dayjs(booking.ngayDat).format('HH:mm DD/MM/YYYY') : '—'}
                          </Typography>
                        </Stack>
        ...
        <DialogContent>
          <Stack spacing={3} sx={{ pt: 1 }}>
            ...
            <TextField
              ...
              onChange={(e) => setEditForm({ ...editForm, xacNhanMatKhau: e.target.value })}
