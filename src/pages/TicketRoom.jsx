...
export default function TicketRoom() {
  ...
  const toggleSeat = (g) => {
    ...
    setPicked((prev) => {
      const cp = { ...prev };
      if (cp[g.maGhe]) delete cp[g.maGhe]; // Nếu ghế đã chọn, bỏ chọn
  ...
  const book = async () => {
    const danhSachVe = Object.values(picked).map((g) => ({ maGhe: g.maGhe, giaVe: g.giaVe }));
  ...
  return (
    <Stack gap={2}>
      <Typography variant="h5" fontWeight={700}>Đặt vé</Typography>
      <Typography color="text.secondary">
        {room.thongTinPhim?.tenCumRap} — {room.thongTinPhim?.tenRap} — {room.thongTinPhim?.tenPhim}
      </Typography>

      <Box sx={{ p: 2, border: '1px dashed rgba(255,255,255,0.2)', borderRadius: 2, maxWidth: 900 }}>
        <Box sx={{ textAlign: 'center', mb: 2, opacity: 0.7 }}>Màn hình</Box>
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(16, 1fr)', gap: 0.5 }}>
            ...
            return (
              <Button
                ...
                sx={{
                  ...
                  height: 32,
                  fontSize: 12,
                  bgcolor: sold ? 'grey.700' : selected ? 'primary.main' : vip ? 'warning.dark' : 'grey.900',
                  opacity: sold ? 0.5 : 1,
                  color: 'white',
                  '&:hover': { bgcolor: sold ? 'grey.700' : selected ? 'primary.dark' : vip ? 'warning.main' : 'grey.800' },
                }}
                disabled={sold}
      ...
      <Stack direction="row" gap={1} alignItems="center">
        ...
        <Chip size="small" label="Đã bán" sx={{ bgcolor: 'grey.700', opacity: 0.6 }} />
        <Chip size="small" label="Đang chọn" sx={{ bgcolor: 'primary.main' }} />
      </Stack>

      <Divider />
      <Stack direction="row" alignItems="center" justifyContent="space-between" maxWidth={900}>
        <Typography>Tổng: <b>{total.toLocaleString()}đ</b> — Ghế: {Object.values(picked).map((g) => g.tenGhe).join(', ') || 'Chưa chọn'}</Typography>
        <Button variant="contained" onClick={book}>Đặt vé</Button>
      </Stack>
    </Stack>
  );
}
