...
reducers: {},
extraReducers: (b) => {
b.addCase(fetchShowtimes.pending, (s)=>{ s.status='loading'; })
...
if(!map.has(p.maPhim)){
map.set(p.maPhim, { maPhim:p.maPhim, tenPhim:p.tenPhim, hinhAnh:p.hinhAnh, lstLichChieuTheoPhim: [] });
}
...
.addCase(fetchShowtimes.rejected, (s,a)=>{ s.status='failed'; s.error=String(a.error?.message||'Error'); });
}
});
