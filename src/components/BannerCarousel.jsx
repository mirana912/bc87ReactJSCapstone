...
const BannerCarousel = memo(function BannerCarousel() {
  ...
  const { items = [], status } = useAppSelector((s) => s.banners);
  ...
  return (
    <Box ref={wrapRef} sx={{ position: 'relative', borderRadius: 4, overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.4)' }}>
      <Swiper
      ...
      >
        {itemsMemo.map((b) => (
          <SwiperSlide key={b.maBanner}>
            <Card sx={{ position: 'relative', borderRadius: 0, overflow: 'hidden', height: { xs: 280, md: 500 } }}>
              <CardMedia
                component="img"
              ...
              <Box
                sx={{
                  position: 'absolute',
                  inset: 0,
                  background: 'linear-gradient(135deg, rgba(0,0,0,0.1) 0%, rgba(0,0,0,0.4) 50%, rgba(0,0,0,0.7) 100%)',
