...
const Showtimes = memo(function Showtimes() {
  ...
  const { items, status } = useAppSelector(s => s.showtimes);
  const { selected } = useAppSelector(s => s.cinemas);
  ...
  if (itemsMemo.length === 0) {
    return <Typography color="text.secondary" py={2}>Chọn hệ thống rạp bên trái để xem lịch chiếu.</Typography>;
  }
            ...
            >
              <Card sx={{ display: 'flex', gap: 2 }}>
                ...
                <CardMedia
                  ...
                  onError={(e) => { e.target.src = 'https://via.placeholder.com/140x200?text=No+Image'; }}
                />
                <Stack sx={{ flex: 1, p: 1 }}>
                  <CardHeader titleTypographyProps={{ variant: 'h6' }} title={m.tenPhim} sx={{ p: 0, pb: 1 }} />
                  <CardContent sx={{ p: 0 }}>
