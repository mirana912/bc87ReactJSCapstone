...
const readCache = (key) => {
  ...
  if ((Date.now() - hit.t) > (hit.ttl || CACHE_TTL)) {
    try { localStorage.removeItem(LS_KEY + key); } catch { /* ignore error */ }
    mem.delete(key);
    return null;
  }
  return hit;
...
const writeCache = (key, data, headers, ttl) => {
  ...
  mem.set(key, val);
  try { localStorage.setItem(LS_KEY + key, JSON.stringify(val)); } catch { /* ignore error */ }
};
...
export const invalidateCache = (pattern) => {
  ...
  keys.forEach(k => {
    mem.delete(k);
    try { localStorage.removeItem(LS_KEY + k); } catch { /* ignore */ }
  });
};
...
client.interceptors.request.use((cfg) => {
  ...
  const bearer = localStorage.getItem('token') || import.meta.env.VITE_API_TOKEN;
...
client.interceptors.response.use((res) => {
  ...
  } else if ((method === 'post' || method === 'put') && res.status >= 200 && res.status < 300) {
