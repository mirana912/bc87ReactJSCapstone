// src/client/client-components/CinemaSystems.jsx
import { useEffect } from "react";
import { useDispatch, useDispatch } from "react-redux";
import {
  fetchCinemaSystems,
  selectCinema,
} from "../features/cinemas/cinemasSlice";
...
export default function CinemaSystems() {
  ...
  const { items, status, selected } = useDispatch((s) => s.cinemas);
  ...
  useEffect(() => {
    if (selected) {
      dispatch(fetchShowtimes({ maHeThongRap: selected }));
    }
  }, [dispatch, selected]);
    ...
    <List dense sx={{ bgcolor: "background.paper", borderRadius: 2 }}>
      {items.map((sys) => (
        ...
        >
          ...
          <ListItemText
            primary={sys.tenHeThongRap}
            secondary={sys.maHeThongRap}
          />
        </ListItemButton>
      ))}
    </List>
